...
export default {
	...
	mounted: function () {
		...
		//  // 获取项目类型列表
		//  this.axios.post("/Index/get_type", {emulateJSON: true}).then(data =>{  
		// 	that.project_type_list = data  
		// });
	...
	methods: {
		...
		getList() {
			...
			if(that.type_id > 0){
				formData.evaluationclass_id = that.type_id 
			}
			...
			that.axios.post("/evaluation_capability/index", formData, {
				emulateJSON: true
			}).then(
				function (res) {
					let data = res.data 
		...
		del_item: function (e) {
			...
			that.$confirm('此操作将永久删除该项, 是否继续?', '提示', {
				...
				var formData = {}
				formData.evaluation_capability_id = e;
				that.axios.post('/evaluation_capability/del', formData, {
					emulateJSON: true
				}).then(
					function (res) {
						if(!res.status){
