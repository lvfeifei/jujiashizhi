...
Page({
    ...
    data: {
        ...
        nav_height: 0,
        question_list: [],
        question: wx.getStorageSync('evaluation_one_data') || []
    },
    ...
    click_item(e) {
        ...
        // 查询"无"的选项ID
        let no_id = 0;                                              
        let option_list = that.data.question_list[index].options;
        if (option_list.length > 0) {
            for(var i in option_list) {
                if (option_list[i].sn == 'N0') {
                    no_id = option_list[i].id;
                }
            }
        }
                ...
                if (i.id == item.capability_id) {
                    ...
                    if (i.options.indexOf(item.id) == -1) {
                        if (item.sn == 'N0') {
                            ...
                            let no_id_i = i.options.indexOf(no_id);
                            if (no_id_i != -1) {
                                i.options.splice(no_id_i, 1)
                            }
                        }
                        if (that.data.question_list[index].type==2) {
    ...
    // 返回上一页
    go_back() {
        wx.setStorageSync('evaluation_one_data', this.data.question || [])
        wx.navigateBack({
            delta: 1,
        })
    },
    ...
    go_jump(e) {
        ...
        if (number == 1) {
            ...
            if (!question.length || question.length < question_list.length) {
                return app.toast_none('每个选项不能为空');
            }
            wx.setStorageSync('evaluation_one_data', question)
            wx.navigateTo({
                url
            })
