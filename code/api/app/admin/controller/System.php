...
class System extends Basic
{
    ...
    public function show()
    {
        ...
        if (!$role_id) {
            json_success(['status'=>0, 'msg'=>'获取权限id错误！']);
        }
        $SystemMenuModel = new \app\admin\model\SystemMenu();
        $system_role = new SystemRole();

        $menu_id = $system_role->where(array('id' => $role_id))->column('menu_id');
        if (!$menu_id) {
            json_success(['status'=>0, 'msg'=>'获取菜单权限错误！']);
        }
        $newArr = [];
        foreach ($menu_id as $k=>$v){
            $newArr[] = json_decode($v);
        }
        $a = array_reduce($newArr, 'array_merge', array());
        
        $list = $SystemMenuModel->show($a);
        if($role_id ==11){
        ...
        if ($list) {
            ...
            json_success(['status'=>1, 'msg'=>'请求成功！','data'=>[]]);
...
//        // 获取菜单图标
    ...
    public function add()
    {
        ...
        if ($id > 0) {
            json_success(['status'=>1, 'msg'=>'系统菜单-添加成功！']);
        } else {
    ...
    public function edit()
    {
        ...
        if ($id !== false) {
            json_success(['status'=>1, 'msg'=>'系统菜单-修改成功！！']);
        } else {
