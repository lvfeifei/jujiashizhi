...
class File
{
    ...
    protected function getMasterLogFile()
    {
        if ($this->config['single']) {
            ...
            if ($this->config['max_files']) {
                ...
                $filename = date('Ym') . DIRECTORY_SEPARATOR . date('Y-m-d') . $cli . '.log';
            }
    ...
    protected function write($message, $destination, $apart = false, $append = false)
    {
        ...
        $info['timestamp'] = date($this->config['time_format'],$sec).".".floor($msec*1000);
    ...
    protected function getDebugLog(&$info, $append, $apart)
    {
        if (App::$debug && $append) {
            ...
            if ($this->config['json']) {
                ...
                $time_str   = '[runtime=' . number_format($runtime, 6) . 's] [reqs=' . $reqs . 'req/s]';
                $memory_str = ' [memory=' . $memory_use . 'kb]';
                $file_load  = ' [file=' . count(get_included_files()) . ']';
